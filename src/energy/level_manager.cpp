...
auto level_manager::load_cosmic_levels(const std::string &levels_path) -> pxe::result<> {
	...
	for(const jsoncons::basic_json<char> &level: parsed.array_range()) {
		cosmic_level cosmic;
		if(const auto err = parse_cosmic_level(level).unwrap(cosmic); err) {
			return pxe::error("failed to parse cosmic level", *err);
		}
...
auto level_manager::parse_cosmic_level(const jsoncons::basic_json<char> &level) -> pxe::result<cosmic_level> {
	...
	if(!level.contains("time") || !level["time"].is_object()) { // NOLINT(*-pro-bounds-avoid-unchecked-container-access)
		return pxe::error("cosmic level entry missing 'ranges' array");
	}
	...
	for(const auto &range: level["ranges"].array_range()) { // NOLINT(*-pro-bounds-avoid-unchecked-container-access)
		cosmic_range new_range{};
		if(const auto err = parse_cosmic_range(range).unwrap(new_range); err) {
			return pxe::error("failed to parse cosmic range", *err);
		}
		cosmic.ranges.push_back(new_range);
	}

	return cosmic;
}

auto level_manager::parse_cosmic_range(const jsoncons::basic_json<char> &range) -> pxe::result<cosmic_range> {
	if(!range.contains("from") || !range["from"].is_uint64() // NOLINT(*-pro-bounds-avoid-unchecked-container-access)
	   || !range.contains("to") || !range["to"].is_uint64()	 // NOLINT(*-pro-bounds-avoid-unchecked-container-access)
	...
	cosmic_range new_range{};
	new_range.from = range["from"].as<uint64_t>();		   // NOLINT(*-pro-bounds-avoid-unchecked-container-access)
	new_range.to = range["to"].as<uint64_t>();			   // NOLINT(*-pro-bounds-avoid-unchecked-container-access)
	new_range.energies = range["energies"].as<uint64_t>(); // NOLINT(*-pro-bounds-avoid-unchecked-container-access)
	new_range.empty = range["empty"].as<uint64_t>();	   // NOLINT(*-pro-bounds-avoid-unchecked-container-access)
	return new_range;
}
